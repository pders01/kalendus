...
describe('LayoutCalculator', () => {
    ...
    describe('Single Events', () => {
        it('should give single event full width and position at origin', () => {
            ...
            expect(box.width).to.equal(100, 'Single event should have 100% width');
            expect(box.x).to.equal(0, 'Single event should be positioned at x=0');
        ...
        it('should handle multiple non-overlapping events', () => {
            ...
            layout.boxes.forEach((box, index) => {
                expect(box.width).to.equal(100, `Non-overlapping event ${index} should have 100% width`);
                expect(box.x).to.equal(0, `Non-overlapping event ${index} should be at x=0`);
                expect(box.depth).to.equal(0, `Non-overlapping event ${index} should have depth 0`);
                expect(box.group).to.equal(index, `Non-overlapping event ${index} should be in separate group`);
            });
        });
    });
    ...
    describe('Overlapping Events', () => {
        it('should identify longest event as background in simple overlap', () => {
            ...
            expect(backgroundEvent!.id).to.equal('2', 'Longest event should be background');
            expect(backgroundEvent!.width).to.equal(100, 'Background event should have 100% width');
            expect(backgroundEvent!.x).to.equal(0, 'Background event should be at x=0');
            expect(overlappingEvent!.width).to.be.lessThan(100, 'Overlapping event should have reduced width');
            expect(overlappingEvent!.x).to.be.greaterThan(0, 'Overlapping event should be offset');
        });
        ...
        it('should handle three overlapping events with correct background assignment', () => {
            ...
            expect(groups.size).to.equal(1, 'All overlapping events should be in same group');

            // Find background event (depth 0)
            const backgroundEvent = layout.boxes.find(box => box.depth === 0);
            expect(backgroundEvent).to.exist;
            expect(backgroundEvent!.id).to.equal('3', 'Longest event should be background');
            expect(backgroundEvent!.width).to.equal(100, 'Background event should have 100% width');
            expect(backgroundEvent!.x).to.equal(0, 'Background event should be at x=0');

            // Check that other events have cascading depths
            const depths = layout.boxes.map(box => box.depth).sort();
            expect(depths).to.deep.equal([0, 1, 2], 'Should have depths 0, 1, 2');
        });
        ...
        it('should handle partial overlaps correctly', () => {
            ...
            const groups = new Set(layout.boxes.map(box => box.group));
            expect(groups.size).to.equal(1, 'All transitively overlapping events should be in same group');

            // Find the longest event (A is 90 minutes, B is 60 minutes, C is 45 minutes)
            const backgroundEvent = layout.boxes.find(box => box.depth === 0);
            expect(backgroundEvent).to.exist;
            expect(backgroundEvent!.id).to.equal('1', 'Event A (longest) should be background');
            expect(backgroundEvent!.width).to.equal(100, 'Background event should have 100% width');
        });
    });
    ...
    describe('Complex Scenarios', () => {
        it('should handle dense overlapping scenario', () => {
            ...
            const depths = layout.boxes.map(box => box.depth).sort();
            expect(depths).to.deep.equal([0, 1, 2, 3, 4], 'Should have depths 0-4');
        });
        ...
        it('should handle cascading overlaps with correct width allocation', () => {
            ...
            const backgroundEvent = layout.boxes.find(box => box.depth === 0);
            expect(backgroundEvent).to.exist;
            expect(backgroundEvent!.id).to.equal('1', 'Workshop (longest) should be background');
            expect(backgroundEvent!.width).to.equal(100, 'Background event should have 100% width');
            expect(backgroundEvent!.x).to.equal(0, 'Background event should be at x=0');
            ...
            overlappingEvents.forEach(event => {
                expect(event.width).to.be.at.least(65, 'Overlapping events should have minimum 65% width');
                expect(event.width).to.be.lessThan(100, 'Overlapping events should have less than 100% width');
                expect(event.x).to.be.greaterThan(0, 'Overlapping events should be offset from left edge');
            });
        });
    });
    ...
    describe('Z-Index Assignment', () => {
        it('should assign z-index based on depth (background behind, overlapping in front)', () => {
            ...
            expect(backgroundEvent!.zIndex).to.be.lessThan(foregroundEvent!.zIndex, 
    ...
    describe('Edge Cases', () => {
        it('should handle events with same start and end times', () => {
            ...
            const groups = new Set(layout.boxes.map(box => box.group));
            expect(groups.size).to.equal(1, 'Identical events should be in same group');
        });
        ...
        it('should handle zero-duration events', () => {
            ...
            expect(box.width).to.equal(100, 'Zero duration event should have 100% width');
            expect(box.x).to.equal(0, 'Zero duration event should be at x=0');
            expect(box.depth).to.equal(0, 'Zero duration event should have depth 0');
        });
    });
    ...
    describe('Layout Validation', () => {
        it('should ensure all background events (depth 0) have 100% width', () => {
            ...
            const scenarios = [
                // Simple overlap
                [
                    { id: '1', heading: 'A', startTime: { hour: 9, minute: 0 }, endTime: { hour: 11, minute: 0 }, color: '#1' },
                    { id: '2', heading: 'B', startTime: { hour: 9, minute: 30 }, endTime: { hour: 10, minute: 0 }, color: '#2' }
                ],
                // Triple overlap
                [
                    { id: '1', heading: 'A', startTime: { hour: 9, minute: 0 }, endTime: { hour: 10, minute: 0 }, color: '#1' },
                    { id: '2', heading: 'B', startTime: { hour: 9, minute: 15 }, endTime: { hour: 11, minute: 0 }, color: '#2' },
                    { id: '3', heading: 'C', startTime: { hour: 9, minute: 30 }, endTime: { hour: 10, minute: 30 }, color: '#3' }
                ],
                // Complex cascading
                [
                    { id: '1', heading: 'A', startTime: { hour: 9, minute: 0 }, endTime: { hour: 12, minute: 0 }, color: '#1' },
                    { id: '2', heading: 'B', startTime: { hour: 9, minute: 30 }, endTime: { hour: 10, minute: 0 }, color: '#2' },
                    { id: '3', heading: 'C', startTime: { hour: 10, minute: 15 }, endTime: { hour: 11, minute: 15 }, color: '#3' },
                    { id: '4', heading: 'D', startTime: { hour: 10, minute: 45 }, endTime: { hour: 11, minute: 0 }, color: '#4' }
                ]
            ];
            ...
            scenarios.forEach((events, scenarioIndex) => {
                ...
                backgroundEvents.forEach((event, eventIndex) => {
                    expect(event.width).to.equal(100, 
        ...
        it('should ensure overlapping events have cascading positions', () => {
            const events = [
                { id: '1', heading: 'Background', startTime: { hour: 9, minute: 0 }, endTime: { hour: 11, minute: 0 }, color: '#1' },
                { id: '2', heading: 'Overlap 1', startTime: { hour: 9, minute: 30 }, endTime: { hour: 10, minute: 0 }, color: '#2' },
                { id: '3', heading: 'Overlap 2', startTime: { hour: 9, minute: 45 }, endTime: { hour: 10, minute: 15 }, color: '#3' }
            ];

            const layout = calculator.calculateLayout(events);
            const sortedByDepth = layout.boxes.sort((a, b) => a.depth - b.depth);
            ...
            for (let i = 1; i < sortedByDepth.length; i++) {
                expect(sortedByDepth[i].x).to.be.greaterThan(sortedByDepth[i-1].x, 
                    `Event at depth ${sortedByDepth[i].depth} should have x > event at depth ${sortedByDepth[i-1].depth}`);
                expect(sortedByDepth[i].width).to.be.lessThan(100, 
