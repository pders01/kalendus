...
type Story = StoryObj<LMSCalendar & { 
  heading?: string; 
  color?: string; 
...
export const NavigateMonths: Story = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    if (header) {
      const headerShadow = header.shadowRoot;
      const nextButton = headerShadow?.querySelector('button[name="next"]') as HTMLButtonElement;
      ...
      if (nextButton) {
        ...
        const prevButton = headerShadow?.querySelector('button[name="previous"]') as HTMLButtonElement;
        if (prevButton) {
