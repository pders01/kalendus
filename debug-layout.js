...
testScenarios.forEach((scenario, index) => {
    ...
    eventDurations.forEach((event) => {
        console.log(
            `  ${event.id}: ${event.heading} (${event.duration} minutes)`,
        );
    });
    ...
    layout.boxes.forEach((box) => {
        ...
        console.log(
            `    Duration: ${duration} min | Depth: ${box.depth} | Group: ${box.group}`,
        );
        console.log(
            `    Width: ${box.width}% | X: ${box.x}% | Z-Index: ${box.zIndex}`,
        );

        // Highlight potential issues
        if (box.depth === 0 && box.width !== 100) {
            console.log(
                `    ⚠️  WARNING: Background event (depth 0) has width ${box.width}% instead of 100%!`,
            );
        }
        if (box.depth === 0 && box.x !== 0) {
            console.log(
                `    ⚠️  WARNING: Background event (depth 0) has x=${box.x}% instead of 0%!`,
            );
        }
        console.log('');
    });
    ...
    console.log(
        `  Groups: ${Math.max(...layout.boxes.map((b) => b.group)) + 1}`,
    );

    // Check for issues
    const problematicBackgroundEvents = backgroundEvents.filter(
        (box) => box.width !== 100 || box.x !== 0,
    );
    if (problematicBackgroundEvents.length > 0) {
        console.log(
            `  ❌ ISSUES FOUND: ${problematicBackgroundEvents.length} background events with incorrect width/position!`,
        );
        problematicBackgroundEvents.forEach((box) => {
            const event = scenario.events.find((e) => e.id === box.id);
            console.log(
                `    - ${box.id} (${event?.heading}): width=${box.width}%, x=${box.x}%`,
            );
        });
    } else {
        console.log(
            `  ✅ All background events have correct width (100%) and position (x=0)`,
        );
    }
});
