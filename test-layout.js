    ...
    findLabelPosition(targetBox, allBoxes, targetIndex) {
        ...
        while (attempts < maxAttempts) {
            ...
            for (let i = 0; i < allBoxes.length; i++) {
                ...
                if (
                    Math.abs(testY - otherLabelY) < labelHeight &&
...
class ASCIIRenderer {
    ...
    renderBoxes(grid, boxes) {
        ...
        boxes.forEach((box, index) => {
            ...
            const endRow = Math.min(
                startRow + Math.max(Math.floor(box.height / 60), 1),
                this.height - 1,
            );
            const startCol = Math.floor((box.x - 90) / 8) + 8; // Scale position after time column
            const endCol = Math.min(
                startCol + Math.floor(box.width / 15),
                this.width - 1,
            );

            console.log(
                `Box ${index}: y=${box.y}, startRow=${startRow}, endRow=${endRow}, startCol=${startCol}, endCol=${endCol}`,
            );
            ...
            for (let row = startRow; row <= endRow; row++) {
                for (let col = startCol; col <= endCol; col++) {
                    if (
                        ...
                        console.log(
                            `Placed character at row=${row}, col=${col}`,
                        );
                    } else {
                        console.log(
                            `Out of bounds: row=${row}, col=${col}, height=${this.height}, width=${this.width}`,
                        );
                    }
                }
            }
    ...
    renderLabels(grid, labels) {
        ...
        labels.forEach((label, index) => {
            ...
            const text = `${indent}${index + 1}. ${label.content} (${
                label.timeStr
            }) [depth: ${depth}]`;
...
Object.entries(testScenarios).forEach(([scenarioName, events]) => {
    ...
    console.log(
        `Events: ${events.length}, Groups: ${
            Math.max(...layout.boxes.map((b) => b.group)) + 1
        }`,
    );
    console.log(
        'Box positions:',
        layout.boxes.map((b) => `(x:${b.x}, depth:${b.depth})`).join(', '),
    );
